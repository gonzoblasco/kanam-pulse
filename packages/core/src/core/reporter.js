...
import { formatBytes, statusLabel } from '../utils/format.js';
...
export function renderTerminal(battery) {
  ...
  lines.push(`  Overall: ${BOLD}${overallColor}${battery.overall}/100 (${statusLabel(battery.status)})${RESET}`);
  lines.push(`  Time:    ${battery.runInfo.elapsedMs} ms | ${battery.runInfo.total} checks`);
  lines.push('');
  ...
  for (const check of battery.checks) {
    ...
    lines.push(`    ${color}${String(score).padStart(3)}/100  ${statusLabel(check.status)}${RESET}`);
  }
  ...
  if (suggestions.length === 0) {
    ...
    for (const s of suggestions) {
      const color = STATUS_COLOR[s.priority === 'high' ? 'critical' : s.priority === 'medium' ? 'attention' : 'good'] || '';
      lines.push(`  ${color}- [${s.check}] ${s.action}${RESET}`);
    }
  }

  lines.push('');
  lines.push(`  ${BOLD}Legend:${RESET} 80+ healthy | 70-79 good | 40-69 attention | <40 critical`);
  lines.push('');
  return lines.join('\n');
}
...
export function renderHistory(history) {
  ...
  const trendColor = trend.direction === 'improving' ? STATUS_COLOR.healthy : trend.direction === 'declining' ? STATUS_COLOR.critical : STATUS_COLOR.good;
  lines.push(`  Runs: ${trend.count} | Latest: ${trend.latest}/100 | Trend: ${trendColor}${trend.direction}${RESET}`);
  if (trend.delta !== null) {
    lines.push(`  Change vs previous: ${trend.delta > 0 ? '+' : ''}${trend.delta}`);
  }
  lines.push('');
  ...
  for (const entry of recent) {
    ...
    lines.push(`  ${date.padEnd(24)} ${String(entry.overall).padStart(3)}/100  ${color}${statusLabel(entry.status)}${RESET}`);
  }
